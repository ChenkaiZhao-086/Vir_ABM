...
# #' @param sd: standard deviation of the log-normal distribution (default is 1)
# MCMC.LogPrior.Comp <- function(comp, sd = 1) {
#   if (any(!is.finite(comp))) {
#     return(-Inf)
#   }
#   a <- comp - mean(comp)
#   return(sum(dnorm(a, mean = 0, sd = sd, log = TRUE)))
# }
...
#   a <- comp - mean(comp)
#   a_new <- a + rnorm(length(a), 0, step1)
#   return(a_new - mean(a_new))
# }
...
#     LogPriorFun <- function(comp) MCMC.LogPrior.Comp(comp, sd = 1)
#   }
...
MCMC.PostProcess <- function(dat, burn_in = 5000, thin = 10) {
  # 若输入是数据框，则将其视为单个链，放入列表中
  if (!is.list(dat)) {
    dat <- list(as.data.frame(dat))
  } else if (!is.list(dat)) {
    stop("dat must be a list of chains or a single data frame.")
  }

  # 后续代码保持不变，但注意列名长度应与实际选取的列数一致
  Chain <- lapply(seq_along(dat), function(i) {
    # 假设每个链至少包含前3列（可根据实际情况调整）
    Chain <- as.mcmc(dat[[i]][, 1:3])
    Chain <- window(Chain, start = burn_in + 1, thin = thin)
    # 根据实际列数设置列名（此处以3列为例）
    colnames(Chain) <- c("comp_IAV", "comp_IBV", "comp_RSV")
    return(Chain)
  })
  ...
  posteriori <- apply(Result, 1, function(x) {
    sprintf("%.6f (%.6f, %.6f)", x[1], x[2], x[3])
  })
