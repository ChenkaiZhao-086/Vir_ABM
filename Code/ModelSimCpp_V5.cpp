        ...
        for (size_t i = 0; i < N; i++) // row
        {
            for (size_t j = 0; j < 8; j++) // column
            {
                ...
                case 0:
                {
                    ...
                    vec inf_states = infe_cases(j) / infe_cases;
                    inf_states = 1 / (1 + exp(-0.1 * inf_states));
                    vec tempro_inf_states = conv_to<vec>::from(agent_status.row(i) != 0); // 定位感染状态（哪些是0，哪些是1）
                    inf_states.elem(find(tempro_inf_states)).fill(0);                     // 将感染状态为0的位置的感染概率置为0
